...
func set_npcs(npcs: Array) -> void:
	...
	for npc_variant in npcs:
		...
		_npc_picker.add_item(npc_name if role.is_empty() else "%s (%s)" % [npc_name, role])
...
func set_navigation(nav: Dictionary) -> void:
	...
	for scene_variant in nav.get("scenes", []):
		...
		_scene_picker.add_item(str(scene.get("label", scene.get("id", ""))))
	...
	for transition_variant in transitions:
		...
		if bool(transition.get("default", false)):
			label += " ★"
		var btn := SessionStyle.button("→ %s" % label, "Enchaîner sur cette scène")
...
func set_enabled(enabled: bool) -> void:
	...
	for btn in [_btn_narrate, _btn_npc, _btn_goto, _btn_advance, _btn_complete]:
